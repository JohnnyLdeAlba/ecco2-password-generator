...
function get_character_record(cfg, cmp) 
{
    ...
    for (index = 0; index < cfg.CHARACTER_TOTAL; index++) 
    {
        ...
        cmp.character[index].character = font_resource[(index * 4) + 0];
        cmp.character[index].class_id = font_resource[(index * 4) + 1];

        cmp.character[index].x = position_x;
        cmp.character[index].y = 0;

        cmp.character[index].w = font_resource[(index * 4) + 2];
        cmp.character[index].h = font_resource[(index * 4) + 3];

        if (cmp.character[index].class_id >= 0)
...
/*
function _get_character_id(cfg, cmp, character)
{
    switch (character)
    {
        case '\u9000': return -2;
...
function _get_word_record(cfg, cmp, string)
{
    var character_id = 0;
...
function get_line_record(cfg, cmp)
{
    ...
    for (index = 0; index < cmp.word_total; index++)
    {
        ...
        if ((word_id == -3) ||
            ...
            (cmp.line[line_index].w + w > cmp.bitmap_width))
        {
            line_index++;        
        ...
        if (class_id == -4 || class_id == -5 || class_id == -6)
        {
            cmp.line[line_index].id = class_id;
            previous_class_id = class_id;
        }
        else
            cmp.line[line_index].id = previous_class_id;
...
function get_character_id(cmp, character)
{
    var index;

    for (index = 0; index < cmp.character_total; index++)
        if (character == cmp.character[index].character) 
            return index;
...
function get_word_record(cfg, cmp, string)
{
    ...
    let word_index = 0;
    let previous_class_id = 0;

    let word = null;

    word = new Array();
    word[word_index] = new t_record();

    // needs to be moved.
    cmp.bitmap_width = cfg.IMAGE_WIDTH - (cfg.PADDING * 2);
    cmp.character_total = cfg.CHARACTER_TOTAL;

    for (index = 0; index < string.length; index++)
    {
        character_id = get_character_id(cmp, string.charAt(index));
        if (character_id == -1) continue;;

        w = cmp.character[character_id].w;
        h = cmp.character[character_id].h;
      
        class_id = cmp.character[character_id].class_id;

        if (class_id < 0 || previous_class_id < 0)
        {
            word_index++;
            word[word_index] = new t_record();
            word[word_index].offset = index;
        }
        else if (word[word_index].w + w >= cmp.bitmap_width)
        {
            word_index++;
            word[word_index] = new t_record();
            word[word_index].offset = index;
        ...
        word[word_index].id = class_id;
        word[word_index].w+= w;

        if (word[word_index].h < h)
            word[word_index].h = h;

        word[word_index].size = index - word[word_index].offset;
        word[word_index].size++;

        previous_class_id = class_id;
    }

    cmp.word = word;
    cmp.word_total = word.length;
}
